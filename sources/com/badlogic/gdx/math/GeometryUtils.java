...
public final class GeometryUtils {
    ...
    public static Vector2 toBarycoord(Vector2 p, Vector2 a, Vector2 b, Vector2 c, Vector2 barycentricOut) {
        ...
        barycentricOut.f158x = ((d11 * d20) - (d01 * d21)) / denom;
        barycentricOut.f159y = ((d00 * d21) - (d01 * d20)) / denom;
        return barycentricOut;
    }

    public static boolean barycoordInsideTriangle(Vector2 barycentric) {
        return barycentric.f158x >= 0.0f && barycentric.f159y >= 0.0f && barycentric.f158x + barycentric.f159y <= 1.0f;
    }

    public static Vector2 fromBarycoord(Vector2 barycentric, Vector2 a, Vector2 b, Vector2 c, Vector2 interpolatedOut) {
        float u = (1.0f - barycentric.f158x) - barycentric.f159y;
        interpolatedOut.f158x = ((a.f158x * u) + (barycentric.f158x * b.f158x)) + (barycentric.f159y * c.f158x);
        interpolatedOut.f159y = ((a.f159y * u) + (barycentric.f158x * b.f159y)) + (barycentric.f159y * c.f159y);
        return interpolatedOut;
    }

    public static float fromBarycoord(Vector2 barycentric, float a, float b, float c) {
        return ((((1.0f - barycentric.f158x) - barycentric.f159y) * a) + (barycentric.f158x * b)) + (barycentric.f159y * c);
    }
    ...
    public static Vector2 quadrilateralCentroid(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, Vector2 centroid) {
        ...
        centroid.f158x = avgX1 - ((avgX1 - (((x1 + x4) + x3) / 3.0f)) / 2.0f);
        centroid.f159y = avgY1 - ((avgY1 - avgY2) / 2.0f);
        return centroid;
    }
    ...
    public static Vector2 polygonCentroid(float[] polygon, int offset, int count, Vector2 centroid) {
        ...
        centroid.f158x = (x + ((x0 + x1) * a)) / (6.0f * signedArea);
        centroid.f159y = y / (6.0f * signedArea);
        return centroid;
    }
    ...
    public static float polygonArea(float[] polygon, int offset, int count) {
        ...
        for (int i = offset; i < n; i += 2) {
            int x2 = (i + 2) % n;
            int y2 = (i + 3) % n;
            area = (area + (polygon[i] * polygon[y2])) - (polygon[x2] * polygon[i + 1]);
        }
        return area * 0.5f;
    }
