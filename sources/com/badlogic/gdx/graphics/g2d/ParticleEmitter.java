...
public class ParticleEmitter {
    ...
    public void update(float delta) {
        this.accumulator += 1000.0f * delta;
    ...
    private boolean updateParticle(Particle particle, float delta, int deltaMillis) {
        ...
        if (this.premultipliedAlpha) {
            ...
            particle.setColor(color[0] * a, color[1] * a, color[2] * a, a * (this.additive ? 0.0f : 1.0f));
        } else {
    ...
    public BoundingBox getBoundingBox() {
        ...
        for (int i = 0; i < n; i++) {
            if (active[i]) {
                ...
                bounds.ext(r.f154x, r.f155y, 0.0f);
                bounds.ext(r.f154x + r.width, r.f155y + r.height, 0.0f);
            }
        }
        return bounds;
    }
