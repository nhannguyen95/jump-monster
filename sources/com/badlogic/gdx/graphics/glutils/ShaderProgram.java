...
public class ShaderProgram implements Disposable {
    ...
    public void setUniformf(String name, Vector2 values) {
        setUniformf(name, values.f158x, values.f159y);
    }

    public void setUniformf(int location, Vector2 values) {
        setUniformf(location, values.f158x, values.f159y);
    }

    public void setUniformf(String name, Vector3 values) {
        setUniformf(name, values.f163x, values.f164y, values.f165z);
    }

    public void setUniformf(int location, Vector3 values) {
        setUniformf(location, values.f163x, values.f164y, values.f165z);
    }

    public void setUniformf(String name, Color values) {
        setUniformf(name, values.f40r, values.f39g, values.f38b, values.f37a);
    }

    public void setUniformf(int location, Color values) {
        setUniformf(location, values.f40r, values.f39g, values.f38b, values.f37a);
    }
    ...
    private void addManagedShader(Application app, ShaderProgram shaderProgram) {
        Array<ShaderProgram> managedResources = (Array) shaders.get(app);
    ...
    public static void invalidateAllShaderPrograms(Application app) {
        if (Gdx.gl20 != null) {
            Array<ShaderProgram> shaderArray = (Array) shaders.get(app);
            if (shaderArray != null) {
                for (int i = 0; i < shaderArray.size; i++) {
                    ((ShaderProgram) shaderArray.get(i)).invalidated = true;
                    ((ShaderProgram) shaderArray.get(i)).checkManaged();
                }
            }
        }
    }
    ...
    public static String getManagedStatus() {
        ...
        Iterator i$ = shaders.keys().iterator();
        while (i$.hasNext()) {
            builder.append(((Array) shaders.get((Application) i$.next())).size);
            builder.append(" ");
        }
        builder.append("}");
