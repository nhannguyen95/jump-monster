...
public class Actor {
    ...
    public boolean fire(Event event) {
        ...
        try {
            ...
            for (i = ancestors.size - 1; i >= 0; i--) {
                ancestorsArray[i].notify(event, true);
                if (event.isStopped()) {
    ...
    public float getTop() {
        return this.f74y + this.height;
    }

    public float getRight() {
        return this.f73x + this.width;
    }
    ...
    public boolean clipBegin() {
        return clipBegin(this.f73x, this.f74y, this.width, this.height);
    }
    ...
    public Vector2 localToParentCoordinates(Vector2 localCoords) {
        ...
        float originY;
        if (rotation != 0.0f) {
            float cos = (float) Math.cos((double) (rotation * 0.017453292f));
            float sin = (float) Math.sin((double) (rotation * 0.017453292f));
            originX = this.originX;
            originY = this.originY;
            float tox = (localCoords.f158x - originX) * scaleX;
            float toy = (localCoords.f159y - originY) * scaleY;
            localCoords.f158x = (((tox * cos) + (toy * sin)) + originX) + x;
            localCoords.f159y = ((((-sin) * tox) + (toy * cos)) + originY) + y;
            ...
            originX = this.originX;
            originY = this.originY;
            localCoords.f158x = (((localCoords.f158x - originX) * scaleX) + originX) + x;
            localCoords.f159y = (((localCoords.f159y - originY) * scaleY) + originY) + y;
        }
        return localCoords;
    }
    ...
    public Vector2 parentToLocalCoordinates(Vector2 parentCoords) {
        ...
        float scaleX = this.scaleX;
        float scaleY = this.scaleY;
        float childX = this.f73x;
        float childY = this.f74y;
        float originX;
        float originY;
        if (rotation != 0.0f) {
            float cos = (float) Math.cos((double) (rotation * 0.017453292f));
            ...
            originX = this.originX;
            originY = this.originY;
            float tox = (parentCoords.f158x - childX) - originX;
            float toy = (parentCoords.f159y - childY) - originY;
            parentCoords.f158x = (((tox * cos) + (toy * sin)) / scaleX) + originX;
            parentCoords.f159y = ((((-sin) * tox) + (toy * cos)) / scaleY) + originY;
        } else if (scaleX == 1.0f && scaleY == 1.0f) {
            parentCoords.f158x -= childX;
            parentCoords.f159y -= childY;
        } else {
            originX = this.originX;
            originY = this.originY;
            parentCoords.f158x = (((parentCoords.f158x - childX) - originX) / scaleX) + originX;
            parentCoords.f159y = (((parentCoords.f159y - childY) - originY) / scaleY) + originY;
        }
        return parentCoords;
    }
    ...
    protected void drawDebugBounds(ShapeRenderer shapes) {
        if (this.debug) {
            ...
            shapes.rect(this.f73x, this.f74y, this.originX, this.originY, this.width - 1.0f, this.height - 1.0f, this.scaleX, this.scaleY, this.rotation);
        }
    }
