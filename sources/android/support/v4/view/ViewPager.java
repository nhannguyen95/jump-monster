...
public class ViewPager extends ViewGroup {
    ...
    public static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C04611());
        Parcelable adapterState;
        ClassLoader loader;
        int position;

        /* renamed from: android.support.v4.view.ViewPager$SavedState$1 */
        static class C04611 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C04611() {
            }
        ...
        if (this.mAdapter != null && !this.mPopulatePending && getWindowToken() != null) {
            ...
            while (curIndex < this.mItems.size()) {
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    this.mAdapter.finishUpdate((ViewGroup) this);
                    sort = this.mDrawingOrder == 0 ? true : DEBUG;
                    if (sort) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int vgrav = lp.gravity & Keys.FORWARD_DEL;
    ...
    protected void onPageScrolled(int position, float offset, int offsetPixels) {
        ...
        if (this.mSeenPositionMax < 0 || FloatMath.ceil(((float) position) + offset) > ((float) this.mSeenPositionMax)) {
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaX) {
        int targetPage = (Math.abs(deltaX) <= this.mFlingDistance || Math.abs(velocity) <= this.mMinimumVelocity) ? (this.mSeenPositionMin < 0 || this.mSeenPositionMin >= currentPage || pageOffset >= 0.5f) ? (this.mSeenPositionMax < 0 || this.mSeenPositionMax <= currentPage + 1 || pageOffset < 0.5f) ? (int) ((((float) currentPage) + pageOffset) + 0.5f) : currentPage - 1 : currentPage + 1 : velocity > 0 ? currentPage : currentPage + 1;
        if (this.mItems.size() <= 0) {
            return targetPage;
        }
